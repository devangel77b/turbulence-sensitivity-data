...
# now load mike
mike <- read.table("mikedata/miketobk.csv",header=TRUE,sep=",")
mike <- mike[,c(3,5,6,7)]
...
fig8c <- ggplot(data=all,aes(x=kcenterfreq,y=U))+geom_point()+geom_line(aes(group=as.factor(spd)))
fig8c <- fig8c+geom_point(data=mike,aes(x=kcenterfreq,y=value))+geom_line(data=mike,aes(x=kcenterfreq,y=value),linetype=3)
...
fig8c <- fig8c+xlab("third-octave-band center wavenumber, \\si{\\per\\meter}")+ylab("signal power, $(\\si{\\meter\\per\\second})^2$")

# save raw stand-in PDF version
pdf("fig8draw.pdf",width=3,height=2,family="Times")
print(fig8c)
dev.off()

# latex version
tikz("fig8d.tex",standAlone=TRUE,width=3,height=2)
