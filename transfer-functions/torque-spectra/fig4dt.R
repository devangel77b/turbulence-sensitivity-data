...
fig4d <- ggplot(data=all,aes(x=as.factor(kcenterfreq),y=TX,colour=as.factor(AR)))+geom_point()+geom_line(aes(group=run))
...
pdf("fig4dtraw.pdf",width=3,height=2,family="Times")
print(fig4d)
dev.off()

# latex version
tikz("fig4dt.tex",standAlone=TRUE,width=3,height=2)
