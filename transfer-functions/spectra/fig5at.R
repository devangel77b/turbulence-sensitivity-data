...
fig5a <- ggplot(data=all,aes(x=frequency,y=TX/1000/U,colour=as.factor(AR)))+geom_line(aes(group=run))
...
fig5a <- fig5a+xlab("$f, \\si{\\hertz}$")+ylab("$E_{t_x}/E_{u_x}, (\\si{\\newton\\meter})^2 (\\si{\\meter\\per\\second})^{-2} \\si{\\per\\hertz}$")

# save raw stand-in PDF version
pdf("fig5atraw.pdf",width=3,height=2,family="Times")
print(fig5a)
dev.off()

# latex version
tikz("fig5at.tex",standAlone=TRUE,width=3,height=2)
