...
fig5b <- ggplot(data=all,aes(x=kfrequency,y=FZ/U,colour=as.factor(AR)))+geom_line(aes(group=run))
...
fig5b <- fig5b+xlab("$k, \\si{\\per\\meter}$")+ylab("$E_{f_x}/E_{u_x}, (\\si{\\newton})^2 (\\si{\\meter\\per\\second})^{-2} \\si{\\meter}$")

# save raw stand-in PDF version
pdf("fig5braw.pdf",width=3,height=2,family="Times")
print(fig5b)
dev.off()

# latex version
tikz("fig5b.tex",standAlone=TRUE,width=3,height=2)
