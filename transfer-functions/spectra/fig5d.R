...
fig5d <- ggplot(data=all,aes(x=as.factor(kcenterfreq),y=FZ/U,colour=as.factor(AR)))+geom_point()+geom_line(aes(group=run))
...
fig5d <- fig5d+xlab("third-octave-band center frequency, \\si{\\per\\meter}")+ylab("signal power, $\\si{\\newton\\squared}/(\\si{\\meter\\per\\second})^2$")


# save raw stand-in PDF version
pdf("fig5draw.pdf",width=3,height=2,family="Times")
print(fig5c)
dev.off()

# latex version
tikz("fig5d.tex",standAlone=TRUE,width=3,height=2)
