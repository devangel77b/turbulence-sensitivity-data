...
fig5c <- ggplot(data=all,aes(x=as.factor(centerfreq),y=FZ/U,colour=as.factor(AR)))+geom_point()+geom_line(aes(group=run))
...
pdf("fig5craw.pdf",width=3,height=2,family="Times")
print(fig5c)
dev.off()

# latex version
tikz("fig5c.tex",standAlone=TRUE,width=3,height=2)
