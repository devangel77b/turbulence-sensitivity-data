...
fig4b <- ggplot(data=all,aes(x=kfrequency,y=FZ,colour=as.factor(AR)))+geom_line(aes(group=run))
...
pdf("fig4braw.pdf",width=3,height=2,family="Times")
print(fig4b)
dev.off()

# latex version
tikz("fig4b.tex",standAlone=TRUE,width=3,height=2)
