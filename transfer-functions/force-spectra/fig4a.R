...
fig4a <- ggplot(data=all,aes(x=frequency,y=FZ,colour=as.factor(AR)))+geom_line(aes(group=run))
...
fig4a <- fig4a+xlab("$f, \\si{\\hertz}$")+ylab("$E_{f_x}, (\\si{\\newton})^2 \\si{\\per\\hertz}$")

# save raw stand-in PDF version
pdf("fig4araw.pdf",width=3,height=2,family="Times")
print(fig4a)
dev.off()

# latex version
tikz("fig4a.tex",standAlone=TRUE,width=3,height=2)
