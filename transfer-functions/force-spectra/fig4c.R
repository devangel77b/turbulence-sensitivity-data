...
fig4c <- ggplot(data=all,aes(x=as.factor(centerfreq),y=FZ,colour=as.factor(AR)))+geom_point()+geom_line(aes(group=run))
...
pdf("fig4craw.pdf",width=3,height=2,family="Times")
print(fig4c)
dev.off()

# latex version
tikz("fig4c.tex",standAlone=TRUE,width=3,height=2)
