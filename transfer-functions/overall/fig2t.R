...
fig2 <- ggplot(data=all,aes(x=sqrt(UU),y=sqrt(TXTX)/1000,colour=as.factor(AR),shape=as.factor(treatname)))+geom_point()
fig2 <- fig2+scale_colour_grey()
#fig2 <- fig2+ylim(0,0.04)+xlim(0,0.6)
fig2 <- fig2+xlab("$u'_{y,rms}, \\si{\\meter\\per\\second}$")+ylab("$t'_{z,rms}, \\si{\\newton\\meter}$")


# save raw stand-in PDF version
pdf("fig2traw.pdf",width=3,height=2,family="Times")
print(fig2)
dev.off()

# preview latex version
tikz("fig2t.tex",standAlone=TRUE,width=3,height=2)
...
all$NDforce <- sqrt(all$TXTX)/(0.5*rho*all$V^2*all$plan.area^1.5)
...
fig2nd <- fig2nd+xlab("$\\times 10^5, \\mbox{Re}$")+ylab("$t'_{z,rms}/(0.5\\rho \\overline{u}^2 A \\sqrt{A})$")
fig2nd <- fig2nd+ylim(0,100)
fig2nd <- fig2nd+scale_x_continuous(breaks=c(0,1e5,2e5,3e5,4e5,5e5),labels=c("$0$","$1$","$2$","$3$","$4$","$5$"))

# save raw stand-in PDF version
pdf("fig2ndtraw.pdf",width=3,height=2,family="Times")
print(fig2nd)
dev.off()

# preview latex version
tikz("fig2ndt.tex",standAlone=TRUE,width=3,height=2)
