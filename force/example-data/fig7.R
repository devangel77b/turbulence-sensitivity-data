...
melted <- subset(melted,(variable %in% c("FZ","FY")))
...
fig7a <- fig7a+ylab("$f, \\si{\\newton}$")
fig7a <- fig7a+facet_grid(variable~.,scales="free")

# save raw stand-in PDF version
pdf("fig7araw.pdf",width=3,height=2,family="Times")
print(fig7a)
dev.off()

# pretty latex version
tikz("fig7a.tex",standAlone=TRUE,width=3,height=2)
...
# make histogram
fig7b <- ggplot(data=melted,aes(value,fill=variable))+geom_histogram(binwidth=0.005)+facet_grid(variable~.,scales="free")
fig7b <- fig7b+scale_fill_grey()
fig7b <- fig7b+xlab("$f, \\si{\\newton}$")
fig7b <- fig7b+ylab("$\\mbox{count}$")

# save raw stand-in PDF version
pdf("fig7braw.pdf",width=3,height=2,family="Times")
print(fig7b)
dev.off()

# pretty latex version
tikz("fig7b.tex",standAlone=TRUE,width=3,height=2)
